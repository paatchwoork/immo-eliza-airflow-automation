...
archive_file = f'gzip -k {workdir}/data/clean/data.csv &&                 mv {workdir}/data/clean/data.csv.gz {workdir}/data/archive/clean_data_prev.csv.gz &&                 rm {workdir}/data/raw/data.csv'
...
push_folder = BashOperator(
        task_id = 'push_folder',
        bash_command = f'cd {workdir} && git add .                 && git commit -m 'Airflow run ended $(date)'                 && git push git@github.com:paatchwoork/immo-eliza-airflow-automation.git main',
        dag = dag_scrape
        )
