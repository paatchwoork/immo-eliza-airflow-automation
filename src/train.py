...
def train(workdir: str, **kwargs):
    """Trains a XGBOOST regression model on the full dataset and stores output."""
    # Load the data
    data = pd.read_csv(f"{workdir}/data/clean/data.csv")
    ...
    model = XGBRegressor(objective='reg:absoluteerror', learning_rate=.05, max_depth=7, n_estimators=1000)
    ...
    with open(f"{workdir}/model/model_specs.txt", 'w') as model_specs:
        model_specs.write(f"{model_name}\n")
        # Evaluate the model
        # R2 evaluation
        train_score = r2_score(y_train, model.predict(X_train))
        model_specs.write(f"Train R² score: {train_score}\n")
        test_score = r2_score(y_test, model.predict(X_test))
        model_specs.write(f"Test R² score: {test_score}\n")

        train_score = mean_squared_error(y_train, model.predict(X_train))
        model_specs.write(f"Train MSE score: {train_score}\n")
        test_score = mean_squared_error(y_train, model.predict(X_train))
        model_specs.write(f"Test MSE score: {test_score}\n")
   ...
   #     #visualizer = ResidualsPlot(model,train_alpha=0.5, test_alpha=0.5) # Initialize the residual plot visualizer
        ...
        model_specs.write("\n")
        model_specs.write('Retraining the model on the full dataset...\n')
        model.fit(pd.concat([X_train, X_test]), pd.concat([y_train, y_test]))

        final_score = r2_score(pd.concat([y_train, y_test]), model.predict(pd.concat([X_train, X_test])))
        model_specs.write(f"Final R² score: {final_score}\n")
        final_score = mean_squared_error(pd.concat([y_train, y_test]), model.predict(pd.concat([X_train, X_test])))
        model_specs.write(f"Final MSE score: {final_score}\n")
    ...
    joblib.dump(artifacts, f"{workdir}/model/{model_name}.joblib")
